  ...
  return (
    <div className="min-h-screen bg-[#103b67] flex items-center justify-center p-4 md:p-8 font-sans">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-3xl bg-white/70 backdrop-blur-2xl rounded-3xl md:rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-white/50 overflow-hidden z-10"
      >
        <div className="absolute top-0 w-full h-1.5 md:h-2 bg-linear-to-r from-indigo-500 via-purple-500 to-cyan-500"></div>

        <div className="relative p-5 md:p-10 lg:p-12 flex flex-col md:flex-row gap-6 md:gap-12">
          
          {/* Photo Section */}
          <div className="flex flex-col items-center md:w-1/3 border-b border-indigo-50 pb-6 md:border-b-0 md:border-r md:pb-0 md:pr-8">
            <motion.div className="relative mb-4 md:mb-6 group" whileHover={{ scale: 1.02 }}>
              <div className="w-28 h-28 md:w-36 md:h-36 lg:w-40 lg:h-40 rounded-full p-1 bg-linear-to-br from-indigo-500 via-purple-500 to-cyan-500 shadow-xl">
            ...
            <h2 className="text-lg md:text-xl lg:text-2xl font-black text-slate-800 tracking-tight text-center wrap-break-word px-2">{name || 'User'}</h2>
          ...
          <div className="flex-1 space-y-4 md:space-y-6">
            ...
            <div className="pt-4 md:pt-8 flex flex-col-reverse sm:flex-row items-center justify-end gap-4 sm:gap-4">
              <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                <motion.button onClick={() => router.push('/dashboard')} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.95 }} className="w-full sm:w-auto px-6 py-3 text-sm font-bold text-slate-500 bg-white border border-slate-200 rounded-xl shadow-sm hover:bg-slate-50 transition-all order-2 sm:order-1">Cancel</motion.button>
                <motion.button onClick={handleSaveToBackend} whileHover={{ scale: 1.02, boxShadow: "0 10px 25px -5px rgba(79, 70, 229, 0.4)" }} whileTap={{ scale: 0.95 }} className="w-full sm:w-auto px-8 py-3 text-sm font-bold text-white bg-linear-to-r from-indigo-600 to-purple-600 rounded-xl shadow-lg hover:shadow-xl transition-all order-1 sm:order-2">Save Changes</motion.button>
              </div>
            </div>
          </div>
        </div>
